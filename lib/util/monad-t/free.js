...
class Freeλʹ {
  ...
  foldMap(f, m) {
    return m.chainRec((next, done, v) => v.cata({
      Pure: x => (0, _ramda.map)(done, m.of(x)),
      Lift: (x, g) => (0, _ramda.map)(xx => done(g(xx)), f(x)),
      Ap: (x, y) => (0, _ramda.map)(done, (0, _ramda.ap)(y.foldMap(f, m), x.foldMap(f, m))),
      Chain: (x, g) => (0, _ramda.map)(xx => next(g(xx)), x.foldMap(f, m))
    }), this);
  }
  cata(d) {}
}
...
class Pureλ extends Freeλʹ {
  ...
  ap(y) {
    return (0, _ramda.map)(f => f(this.x), y);
  }
  chain(f) {
    return f(this.x);
  }
...
class Apλ extends Freeλʹ {
  ...
  map(f) {
    return new Apλ(this.x, (0, _ramda.map)(a => (0, _ramda.map)(f, a), this.y));
  }
  toString() {
    return 'Free.Ap';
  }
...
class Chainλ extends Freeλʹ {
  ...
  map(f) {
    return new Chainλ(this.x, a => (0, _ramda.map)(f, this.f(a)));
  }
  ap(y) {
    return new Apλ(this, y);
  }
  ...
  chain(f) {
    return new Chainλ(this.x, v => (0, _ramda.chain)(f, this.f(v)));
  }
  /*::
...
var Free = exports.Free = Object.assign(function Free() {}, {
  Pure,
  of: Pure,
  liftF,
  Ap,
...
function chainRec(f, i) {
  return (0, _ramda.chain)(({ done, value }) => done ? new Pureλ(value) : chainRec(f, value), f(chainRecNext, chainRecDone, i));
}
