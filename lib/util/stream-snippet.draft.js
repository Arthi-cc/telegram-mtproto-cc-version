...
var makeName = (name, index) => (0, _path.join)('.', DIR, ...name, [index.toString(10), '.json'].join(''));

var makeSnippet = name => {
  (0, _fsExtra.ensureDirSync)((0, _path.join)('.', DIR, ...name));
  return (filter, epic) => action$ => {
    ...
    mcast.zip((begin, end) => ({ begin, end }), raw).zip((data, count) => Object.assign({}, data, { count }), counter).observe(({ begin, end, count }) => {
      ...
      (0, _fsExtra.writeFileSync)(fullName, stringify);
    });
    return raw;
  };
};
